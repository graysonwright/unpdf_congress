...
describe Processors::CongressionalRecord do
  describe ".pull_tables" do
    it "preserves tables found in the record" do
      page = File.read(File.dirname(__FILE__) + "/../examples/crec-2020-10-01-p184.txt")
  
      parsed = Processors::CongressionalRecord.pull_tables(page)
      expect(parsed).to eq({
        0 => <<-GRID.rstrip,
...
=begin
  it "identifies sections marked by the 'SECTION' keyword" do
    expect(Processors::CongressionalRecord.process(<<-DOC)).
